...
ginkgoinfo=`ginkgo version`
if [ "$nulldata" = "$ginkgoinfo" ]
then
...
composeinfo=`docker-compose version`
if [ "$nulldata" = "$composeinfo" ]
then 
	case "$OS" in
   		 Ubuntu)
   		 ;;
